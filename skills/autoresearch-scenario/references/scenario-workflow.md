...
Read `../../autoresearch/references/shared-contract.md` first. Its campaign,
lifecycle, delegation, and evidence rules override conflicting examples below.
...
**Blocking prerequisite:** If the seed scenario cannot be inferred safely, use the current environment's available clarification mechanism before exploration.
...
| 6 | `Depth` | "How deep should I explore?" | Always | "Shallow scan (10 iterations  quick overview)", "Standard exploration (25 iterations  recommended)", "Deep investigation (50+ iterations  comprehensive)", "Unbounded budget  continue while explicitly RUNNING" |
...
**Important:** Prefer one compact numbered batch so the user can make related decisions together.
...
- If the user cancels during clarification, exit cleanly and leave the campaign stopped or uncreated.
...
**Gate:** Do not enter this phase until the seed scenario and required boundaries are resolved.
...
**Append a ledger event and render this `scenario-results.tsv` view:**
```tsv
...
**When to stop (unbounded-budget mode):**
- Stop launching immediately when lifecycle leaves `RUNNING`; reconcile active work
  according to the shared contract.
- Print a diminishing-returns warning after 5 iterations with no new situations,
  but do not use it as authorization for more work.
