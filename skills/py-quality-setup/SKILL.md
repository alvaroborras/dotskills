...
This is a foundation skill. Use it directly when the missing piece is project tooling; for broader multi-step refactoring, prefer `py-refactor`.
...
When using `pyrightconfig.json` for multi-package projects, REMOVE the `tool.basedpyright` section from pyproject.toml
...
6. **Codex note**
   ...
   Codex does not use the upstream `.claude/settings.local.json` permission file or Claude-specific stop-hook settings. Keep this skill focused on project tooling: `pyproject.toml`, `pyrightconfig.json`, and the project virtual environment.
...
- [ ] If project-level enforcement is desired, see `py-git-hooks`

## Examples

**Example: New project setup**
```
1. Create pyproject.toml with [project], [tool.ruff], [tool.mypy], [tool.basedpyright]
2. Install: uv sync
3. Verify: uv run ruff --version && uv run mypy --version && uv run basedpyright --version
4. Run initial checks: uv run ruff check . && uv run mypy . && uv run basedpyright .
5. Fix issues or add ignores for false positives
```

**Example: Add quality tools to existing project**
```
1. Update pyproject.toml:
   - Add ruff, mypy, basedpyright to [dependency-groups] dev
   - Add [tool.ruff], [tool.mypy], [tool.basedpyright] sections
2. Install: uv sync
3. Run checks: uv run ruff check . (expect many errors initially)
4. Auto-fix: uv run ruff check --fix . && uv run ruff format .
5. Run type checkers: uv run mypy . && uv run basedpyright .
6. Add [[tool.mypy.overrides]] for third-party packages without stubs
