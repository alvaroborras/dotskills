...
- **`gepa`** — the GEPA optimizer: reflective evolutionary search, in-process (an LLM reflects on
  ...
  is rich. Works with any LiteLLM provider (OpenAI, Bedrock, local gateways, …).
- **`autoresearch`** — an agentic optimizer: one coding-agent subprocess iterates like a researcher in
  a work dir, scoring candidates through an HTTP eval server. This skill provides a Codex CLI bridge.
- **`meta_harness`** — an agentic proposer (coding-agent subprocess) that reads the frontier/history each
  iteration and writes new candidates for the engine to benchmark. This skill provides a Codex CLI bridge.
...
### Proposer LLM (OpenAI GPT-5.6 family recommended)
The gepa backend's reflection LM defaults to `"openai/gpt-5.6-luna"` — set `OPENAI_API_KEY`:
```bash
export OPENAI_API_KEY="sk-..."
```
You can also use any other GPT-5.6 tier or a different provider entirely:
```python
"reflection_lm": "openai/gpt-5.6-luna",   # cost-efficient (default)
"reflection_lm": "openai/gpt-5.6-terra",  # balanced
"reflection_lm": "openai/gpt-5.6-sol",    # flagship
"reflection_lm": "openai/gpt-5.6-luna",   # cost-efficient default
```
Or pass any callable implementing GEPA's LM protocol — a self-hosted / custom inference engine —
instead of a model-id string.

### Optional: Agentic backends through Codex CLI
The `autoresearch` and `meta_harness` backends use this skill's `bin/gepa-agent` adapter, which
translates the engine's print-oriented subprocess contract into `codex exec --json`. Install and
authenticate Codex, then set `sandbox=False`:
```bash
codex --version
python scripts/preflight.py --engine autoresearch
```
The adapter uses `gpt-5.6-luna` by default. Override it with `GEPA_CODEX_MODEL` and
`GEPA_REASONING_EFFORT`.
...
2. **`evaluate(candidate, example) -> (score, info)`** — `score` is a float (higher is better);
   ...
   `references/api.md`.
3. **Engine (backend)** — `"gepa"` (default), `"autoresearch"` (Codex adapter),
   `"meta_harness"` (Codex adapter), the `"best_of_n"` baseline, or a constructed `Engine`
   instance.
...
## Running with Codex instead of a provider CLI
The `gepa` backend uses a LiteLLM model string for the reflection LM. The agentic `autoresearch` and
`meta_harness` backends are also available through this skill's Codex adapter, which translates their
print-oriented subprocess calls into `codex exec --json`. Use `sandbox=False` for those engines.

### GPT-5.6 family (recommended)
OpenAI's GPT-5.6 family offers three tiers, all with 1.05M context and reasoning effort support:

| Tier | LiteLLM id | Input $/1M | Output $/1M | Best for |
|---|---|---|---|---|
| **Luna** | `openai/gpt-5.6-luna` | $0.20 | $1.20 | Reflection LM — cheap iteration |
| **Terra** | `openai/gpt-5.6-terra` | $2.50 | $15.00 | Reflection when quality > cost |
| **Sol** | `openai/gpt-5.6-sol` | $5.00 | $30.00 | Hardest tasks, or as the task LM |

```python
engine_config={
    "reflection": {
        "reflection_lm": "openai/gpt-5.6-luna",   # or terra/sol
        "reflection_lm_kwargs": {"reasoning_effort": "high"},  # none/low/medium/high/xhigh/max
        "reflection_minibatch_size": 5,
    },
    "engine": {"max_workers": 32, "seed": 0},
}
```

Set `OPENAI_API_KEY` for the in-process paths. For agentic paths, authenticate the `codex` CLI.
...
- **`max_token_cost`** — a hard USD cap on the backend's own proposer/agent LLM spend. Especially
   important for agentic backends (`autoresearch`, `meta_harness`), whose subprocess agents spend
   tokens between eval calls.
...
4. **Pick a proposer LLM** — a LiteLLM id (set the provider key) or a custom LM-protocol callable.
   The GPT-5.6 family is recommended: `openai/gpt-5.6-luna` (cost-efficient), `openai/gpt-5.6-terra`
   (balanced), or `openai/gpt-5.6-sol` (flagship). Set `reasoning_effort` in
   `reflection_lm_kwargs` for better reflection quality. Validate it with a 1-call test before a
   long run.
...
- **Agentic backends (`autoresearch`, `meta_harness`) use the skill-owned Codex bridge** and need
   `codex` on PATH. Pass `sandbox=False`; the bridge invokes Codex non-interactively. Run
   `scripts/preflight.py --engine autoresearch` first; details in `references/api.md`.
...
- `scripts/preflight.py` — validate creds / proposer LM / Codex bridge before launching.
