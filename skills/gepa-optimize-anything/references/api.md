...
| `sandbox` | **`True`** | OS-jail subprocess engines when supported by their native adapter. For the skill-owned Codex adapter, set `False`: it invokes Codex non-interactively with an explicit bypass policy. |
...
|---|---|---|---|
| `gepa` | an LLM reflects on evaluator feedback and mutates the candidate; keeps a Pareto frontier | in-process | reflection-LM creds (default `openai/gpt-5.6-luna`) or a custom LM |
| `autoresearch` | one Codex subprocess iterates in a work dir (`program.md`, `candidate.txt`, `eval.sh` → HTTP eval server) | subprocess | `codex` on PATH + authentication, `jq`, `sandbox=False` |
| `meta_harness` | a Codex subprocess reads frontier/history and writes `pending_eval.json` candidates; the engine benchmarks each | subprocess | `codex` on PATH + authentication, `sandbox=False` |
| `best_of_n` *(baseline)* | independent single-shot samples from one LLM; keep the best — no feedback, no history | in-process | LiteLLM creds for `model` (default `openai/gpt-5.6-luna`) |
...
engine_config={
    "reflection": {                     # -> ReflectionConfig
        "reflection_lm": "openai/gpt-5.6-luna",  # use terra/sol for harder tasks
        ...
        # "custom_candidate_proposer": ...,       # optional custom proposer callable
...
| key | default | meaning |
|---|---|---|
| `model` | `"gpt-5.6-luna"` | Codex model id used by the skill adapter. |
| `ralph` | `True` | keep spawning Codex proposer sessions while budget remains; the adapter uses independent sessions. |
...
| `effort` | `None` | reasoning effort passed to the Codex adapter. |
...
launches the skill's Codex adapter, which translates the print contract into `codex exec --json`.
`eval.sh` POSTs candidates to the eval server, which enforces the budget
...
| key | default | meaning |
|---|---|---|
| `model` | `"gpt-5.6-luna"` | Codex proposer model id. |
...
| key | default | meaning |
|---|---|---|
| `model` | `"openai/gpt-5.6-luna"` | LiteLLM model id used to sample candidates. |
...
- a **model-id string** resolved through LiteLLM (`"openai/gpt-5.6-luna"` — the default —
  `"openai/gpt-5.6-terra"`, `"openai/gpt-5.6-sol"`, a Bedrock inference-profile ARN, …); set the provider's
...
result = optimize_sequential(
    ...
    objective="...",
    configs=[
        OptimizeAnythingConfig(engine="autoresearch", max_evals=100, max_token_cost=5.0, sandbox=False),
