...
Read `../../autoresearch/references/shared-contract.md` first. Its runtime,
...
**Blocking prerequisite:** If scope or symptom is missing and cannot be inferred safely, use the current environment's available clarification mechanism before experimentation.
...
When clarification is required, batch these four questions in one interaction when the available mechanism supports it:
...
**Gate:** Do not enter this phase until scope and symptom are resolved.
...
| **Isolated revision search** | Compare known revisions in disposable worktrees | "It used to work" |
...
- Run state-mutating experiments in a disposable sandbox or isolated worktree; never stash or overwrite user work automatically
...
**Append a ledger event and render this `debug-results.tsv` view:**
```tsv
...
- snapshot the current dirty-state manifest and use an isolated worktree for a clean-state comparison
- compare explicitly named revisions in disposable worktrees to narrow the breaking range
