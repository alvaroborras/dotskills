...
run-workflow <script.js>
  ...
  --pin-model M       pin ALL agents across all roles to M (legacy single-model mode)
  ...
  --pin-effort E      force ALL agents to effort E (legacy single-effort mode)
...
layer that spawned it. Without `--auto-effort`, effort comes from the per-role
defaults (orchestrator `ultra`, subagent `high`, scout `medium`). Effort precedence (highest
first): `--pin-effort` → per-call `opts.effort` → `--auto-effort` (which beats
the per-role defaults) → the three `--pin-effort-*` flags
→ `--effort` → Codex config default. The *effective* effort is folded into each
agent's journal identity, so toggling the policy between runs busts only the
agents whose effort changed.
