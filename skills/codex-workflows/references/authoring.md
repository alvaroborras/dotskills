...
| `role` | `"orchestrator"` \| `"subagent"` \| `"scout"` (default `"subagent"`). Sets the role-pinned model and effort: orchestrator = `gpt-5.6-sol`/`ultra`, subagent = `gpt-5.6-luna`/`high`, scout = `gpt-5.6-luna`/`medium`. Use scouts for bounded read-only extraction/classification and orchestrators for lone synthesis/judge/controller gates. |
| `model` | **Leave unset in scripts.** The per-role pinned model overrides any per-call `model` anyway. (If you do set it, Claude ids/aliases auto-map to a Codex model.) |
...
| `effort` | `none`/`minimal`/`low`/`medium`/`high`/`xhigh`/`ultra`. **Usually leave unset** — the per-role defaults are `ultra` for orchestrator, `high` for subagent, and `medium` for scout. Run with `--auto-effort` to override them by layer width (1→`xhigh`, 2+→`high`). A per-call effort overrides that policy. Precedence: `--pin-effort` > per-call `effort` > `--auto-effort` > the three per-role effort pins > `--effort` > Codex config default. |
...
- **Three roles — Sol orchestrator, Luna subagent, Luna scout.** Orchestrators
  get `gpt-5.6-sol`/`ultra`; substantive workers get
  `gpt-5.6-luna`/`high`; bounded read-only scouts get
  `gpt-5.6-luna`/`medium`. The default role is `subagent`; opt into
  `role: "orchestrator"` or `role: "scout"` explicitly. Leave `model` and
  `effort` out of calls and use the three per-role model/effort pin flags when
  overriding. `--frontier` detects the Sol/Luna profile and never auto-selects
  Terra; an explicit pin can still select any exposed model.
