...
1. **Preflight** — once per session, or whenever a run fails to connect, confirm
   ...
   From that list, confirm the default role profile is exposed:
   `gpt-5.6-sol` for orchestrators and `gpt-5.6-luna` for both subagents and
   scouts. `--frontier` auto-detects this Sol/Luna profile without selecting
   Terra — see *Model*.
...
4. **Choose run settings** (see *Run defaults*): `--frontier` (or accept the
   per-role defaults — `gpt-5.6-sol`/`ultra` for orchestrators,
   `gpt-5.6-luna`/`high` for subagents, and `gpt-5.6-luna`/`medium` for scouts),
   the effort flag for the chosen scale
   (`--auto-effort` for a standard/deep harness, `--pin-effort-subagent medium`
   for a quick one), a read-only sandbox unless the run must write, and a
   bounded `--budget`. For an expensive or complex workflow, **`--plan`
   first** — a no-token dry run that counts agents per phase/effort and
   estimates a `--budget`.

5. **Run** it — the per-role defaults already cover most cases. Override with
   `--frontier` when you want the runner to auto-pick the strongest available
   role profile from `model/list`, or use the three `--pin-model-*` flags to
   lock in a specific profile. Add `--auto-effort` to override the per-role
   effort defaults and instead scale by layer width (lone synthesis/judge gates
   get `xhigh`, fan-outs floor at `high`); for a quick harness, override the
   subagent floor with `--pin-effort-subagent medium`; scouts already default
   to medium:
   ```bash
   node ~/.agents/skills/codex-workflows/runner/bin/run-workflow.js <script.js> [flags]
   #   standard: … --frontier --auto-effort [other flags]
   #   quick:    … --pin-effort-subagent medium [other flags]
   #   legacy:   … --pin-model M --pin-effort E [other flags]
   ```
...
| Model — orchestrator role | `--pin-model-orchestrator gpt-5.6-sol` *(default)* | the strongest available model for lone synthesis/judge gates |
| Model — subagent role | `--pin-model-subagent gpt-5.6-luna` *(default)* | Luna for substantive fan-out workers |
| Model — scout role | `--pin-model-scout gpt-5.6-luna` *(default)* | Luna for read-only extraction, classification, and scans |
| Model — auto-detect | `--frontier` | detect Sol for orchestrators and Luna for subagents/scouts; never auto-select Terra |
| Model — one for all (legacy) | `--pin-model M` | forces every agent across all roles to one model — reproducible single-model runs |
| Effort — orchestrator role | `--pin-effort-orchestrator ultra` *(default)* | maximum reasoning for critical single-agent gates |
| Effort — subagent role | `--pin-effort-subagent high` *(default)* | hard floor for fan-out workers |
| Effort — scout role | `--pin-effort-scout medium` *(default)* | cost-efficient effort for bounded scouting and extraction |
| Effort — layer-scaled | `--auto-effort` | scales by parallel width: 1→`xhigh`, 2+→`high` (overrides per-role default) |
| Effort — one for all (legacy) | `--effort E` / `--pin-effort E` | flat fallback for the whole run |
| Effort — `quick_harness` | `--pin-effort-subagent medium` | a small analytical run doesn't need the default high fan-out; cheap it further |
...
## Model: Sol orchestrator + Luna subagent/scout roles

Route each `agent()` and `agent.start()` to one of three roles. Orchestrators
use Sol; both worker roles use Luna. Keep `subagent` and `scout` distinct because
they carry different effort defaults, semantic intent, and journal attribution.

| Role | Default model | Opt on `agent()` | Set when |
|------|---------------|------------------|----------|
| **orchestrator** | `gpt-5.6-sol` | `role: "orchestrator"` | the agent is a critical lone gate — a synthesis, judge, controller, final report — where one weak output sinks the whole run |
| **subagent** | `gpt-5.6-luna` *(default)* | `role: "subagent"` | substantive fan-out work: implementation, deep analysis, adversarial verification |
| **scout** | `gpt-5.6-luna` | `role: "scout"` | bounded read-only extraction, inventory, classification, and evidence scans |

Declare the role explicitly with `role` (default `subagent`); do not infer it
from the phase. Use `role: "scout"` for narrowly bounded scouting work and
`role: "orchestrator"` for critical lone gates.

Three ways to override the defaults:

1. **`--frontier`** *(recommended when your local Codex exposes newer models)*:
   detect the latest Sol/Luna role profile from `model/list`. Never select Terra
   automatically. Stamp all three resolved roles into the run-meta sidecar.
2. **Per-role pins**: use `--pin-model-orchestrator M`,
   `--pin-model-subagent M`, and `--pin-model-scout M` to override any role.
3. **`--pin-model M`** *(legacy single-model mode)*: force every agent across all
   roles to one model. Useful for reproducible single-model runs (a benchmark
   that demands it, a smoke test, a debug session). The CLI warns when you
   combine this with the per-role flags; the per-role flags win.

Also good practice: don't set a per-call `model` in scripts — leave `model` out
of every `agent()` opts object. The runner's role-aware pin overrides any
per-call `model` for the agent's role; setting one only adds noise. (If your
Codex install doesn't yet expose `gpt-5.6-sol`/`gpt-5.6-luna`, `resolveModel`
warns and falls back to the Codex config default — at which point
the per-role pin flags or `--frontier` are the
correct response.)

Need to bound cost? Lower effort (see below) and set `--budget` — do not switch
roles mid-run and do not collapse to one model.

## Effort: per-role defaults + layer-width scaling

Thinking effort is the second dial. Orchestrators default to `ultra`,
subagents to `high`, and scouts to `medium`; this keeps bounded scouting cheaper
without weakening substantive worker or critical-gate reasoning.

**Default per-role effort** (no flags needed):

| Role | Default effort | Why |
|------|----------------|-----|
| **orchestrator** | `ultra` | maximum reasoning on lone synthesis / judge / report / controller gates |
| **subagent** | `high` | hard floor for fan-out workers — covers most cases without overpaying |
| **scout** | `medium` | bounded extraction and classification without worker-level reasoning spend |

Override per-role with the corresponding `--pin-effort-*` flag, or globally
with `--pin-effort E` (legacy single-effort pin, wins for all roles). Add
`--auto-effort` to override the per-role defaults and instead scale
effort by **layer width** — the principle that a lone agent in its layer is a
critical gate and earns maximum reasoning, while a 12-wide fan-out gets a
cheaper-but-still-hard floor:
...
With `--auto-effort`, a forge-style run's `consolidate`, `portfolio-judge`, and
`report-writer` agents get `xhigh`; the 3–4-wide pain/mechanism/recombination
waves *and* the 12-persona / 8-critic fan-outs all get `high`. The floor is
`high`; the policy never drops to `medium`. No per-agent bookkeeping.

Precedence (highest first): **`--pin-effort E`** (force every agent to `E`) →
a script's **per-call `effort`** → **`--auto-effort`** layer policy (which
beats the per-role defaults — passing `--auto-effort` opts out of them) →
the three **`--pin-effort-* E`** flags (per-role
default baseline) → flat **`--effort E`** → Codex config default. Because
per-call effort overrides the policy, **do not hand-set `effort` in scripts**
— leave it out and let the per-role defaults (or `--auto-effort`) govern;
reserve a per-call `effort` for a rare, deliberate exception (e.g. forcing
`xhigh` on one unusually hard agent *inside* a wide layer).

Bound cost without touching the model: keep the per-role defaults but add a
`--budget` backstop, or drop subagents a tier with `--pin-effort-subagent
medium`. For a small `quick_harness`, flat **`--pin-effort-subagent medium`**
(or `--effort medium`) is the right default — cheaper, uniform, and the
orchestrator gates still get their `ultra` thinking.
...
`role` (`"orchestrator"` | `"subagent"` | `"scout"`, default `"subagent"` — see *Model*; sets
the per-role pinned model for this agent), `model` (Claude ids/aliases auto-map
to a Codex model — usually omit; the per-role pin overrides it), `agentType`
(loads `.claude/agents/<name>.md` as the system prompt), `systemPrompt`, `effort`
(usually omit — let the per-role default or `--auto-effort` govern; see *Effort*),
...
run-workflow <script.js>
  ...
  --frontier       auto-detect orchestrator→Sol and subagent/scout→Luna from
                   model/list; never auto-select Terra; overrides per-role pins
  --pin-model M    pin ALL agents across all roles to model M — legacy single-model
  --pin-model-orchestrator M   pin role:'orchestrator' agents to M (default gpt-5.6-sol)
  --pin-model-subagent M       pin role:'subagent' agents to M (default gpt-5.6-luna)
  --pin-model-scout M          pin role:'scout' agents to M (default gpt-5.6-luna)
  --model M        fallback model when not pinned; Claude ids/aliases auto-map
  --effort E       none|minimal|low|medium|high|xhigh|ultra; flat fallback;
                   unset → per-role default (orchestrator ultra, subagent high,
                   scout medium)
  --auto-effort    scale effort to layer width: 1→xhigh, 2+→high (floor)
                   (overrides per-role default)
  --pin-effort E   force ALL agents to effort E, overriding per-call effort
                   — legacy single-effort
  --pin-effort-orchestrator E  pin orchestrator agents to E (default ultra)
  --pin-effort-subagent E      pin subagent agents to E (default high)
  --pin-effort-scout E         pin scout agents to E (default medium)
  ...
  --notify-cmd C   run shell command (detached, best-effort; event JSON in
...
- **Sizing `--budget`** — it is a *hard ceiling that throws mid-run*, not an
  ...
  of thumb: medium-effort Luna worker (`gpt-5.6-luna`) spends
  **~0.3–0.5M tokens/agent** (reasoning included); orchestrator-effort (`ultra`)
  agents spend more — plan for `~1M/agent` for orchestrator gates and
  `~0.5M/agent` for subagent workers, so an N-agent run wants
  `--budget ≈ orchestrator_agents × 1M + subagent_agents × 500k` with headroom.
  (A 35-agent run blew past an 8M ceiling after only ~17 agents.)
...
- **Model mapping** — a script that requests `claude-opus-4-8` or a bare
  ...
  that: accept the per-role defaults (`gpt-5.6-sol`/`gpt-5.6-luna`) or pin
  with `--frontier` (auto-detect) / `--pin-model-orchestrator M` /
  `--pin-model-subagent M` / `--pin-model-scout M` / `--pin-model M` — see *Model*. (`--model` is only
  the *fallback* default; a per-call `model` in the
