...
import { pickFrontierRoles } from "../src/modelMap.js";
...
function parseArgs(argv) {
  const out = {
    ...
    model: null,
    pinModel: null,
    pinModelOrchestrator: null,
    pinModelSubagent: null,
    pinModelScout: null,
    frontier: false,
    sandbox: null,
    effort: null,
    autoEffort: false,
    pinEffort: null,
    pinEffortOrchestrator: null,
    pinEffortSubagent: null,
    pinEffortScout: null,
    budgetMeter: "total",
    plan: false,
    tui: false,
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    else if (a === "--pin-model-orchestrator") out.pinModelOrchestrator = rest[++i];
    else if (a === "--pin-model-subagent") out.pinModelSubagent = rest[++i];
    else if (a === "--pin-model-scout") out.pinModelScout = rest[++i];
    ...
    else if (a === "--pin-effort-orchestrator") out.pinEffortOrchestrator = rest[++i];
    else if (a === "--pin-effort-subagent") out.pinEffortSubagent = rest[++i];
    else if (a === "--pin-effort-scout") out.pinEffortScout = rest[++i];
...
if (opts.help || !opts.script) {
  console.error(
    "usage: run-workflow <script.js> [--args JSON] [--args-file path]\n" +
      "  [--budget N] [--budget-meter total|output] [--model M]\n" +
      "  [--frontier | --pin-model M | --pin-model-orchestrator M\n" +
      "   --pin-model-subagent M --pin-model-scout M]\n" +
      "  [--effort none|minimal|low|medium|high|xhigh|ultra]\n" +
      "  [--auto-effort | --pin-effort E\n" +
      "   | --pin-effort-orchestrator E --pin-effort-subagent E --pin-effort-scout E]\n" +
      ...
      "  --frontier       auto-detect the Sol/Luna role profile from model/list:\n" +
      "                   orchestrator→Sol, subagent→Luna, scout→Luna. Terra is not\n" +
      "                   selected automatically. Overrides per-call models.\n" +
      "  --pin-model M    pin ALL agents to a single model M, overriding per-call model\n" +
      "                   (legacy single-model mode; use the per-role pin flags for\n" +
      "                   the recommended role split)\n" +
      "  --pin-model-orchestrator M\n" +
      "                   pin agents with role:'orchestrator' to model M (default\n" +
      "                   gpt-5.6-sol). Overrides a per-call model on orchestrators.\n" +
      "  --pin-model-subagent M\n" +
      "                   pin agents with role:'subagent' (the default) to model M\n" +
      "                   (default gpt-5.6-luna). Overrides a per-call model on\n" +
      "                   subagents.\n" +
      "  --pin-model-scout M\n" +
      "                   pin agents with role:'scout' to model M (default\n" +
      "                   gpt-5.6-luna). Overrides a per-call model on scouts.\n" +
      ...
      "                   Overridden by a per-call effort or by a per-role pin;\n" +
      "                   overrides --effort.\n" +
      "  --pin-effort E   force ALL agents to effort E, overriding per-call effort\n" +
      "                   (legacy single-effort mode; use the per-role effort flags\n" +
      "                   for the recommended role split)\n" +
      "  --pin-effort-orchestrator E\n" +
      "                   force orchestrator-role agents to effort E (default ultra).\n" +
      "  --pin-effort-subagent E\n" +
      "                   force subagent-role agents to effort E (default high).\n" +
      "  --pin-effort-scout E\n" +
      "                   force scout-role agents to effort E (default medium).\n" +
...
// `ultra` (the default for orchestrator-role agents) sits above `xhigh`.
const EST_TOKENS_PER_EFFORT = {
  none: 80_000, minimal: 80_000, low: 150_000, medium: 350_000, high: 550_000, xhigh: 800_000, ultra: 1_100_000,
};
...
// fallback. The per-role pin flags override the legacy single-effort pin for
// their role.
// Validate effort spellings up front so a typo fails fast.
const EFFORTS = new Set(["none", "minimal", "low", "medium", "high", "xhigh", "ultra"]);
for (const [flag, val] of [
  ["--effort", opts.effort],
  ["--pin-effort", opts.pinEffort],
  ["--pin-effort-orchestrator", opts.pinEffortOrchestrator],
  ["--pin-effort-subagent", opts.pinEffortSubagent],
  ["--pin-effort-scout", opts.pinEffortScout],
]) {
...
// Per-role effort defaults: orchestrator → ultra, subagent → high, scout →
// medium. Each can be overridden by its per-role flag or a per-call effort.
const DEFAULT_EFFORT_ORCHESTRATOR = "ultra";
const DEFAULT_EFFORT_SUBAGENT = "high";
const DEFAULT_EFFORT_SCOUT = "medium";

// Per-role model defaults: orchestrator → Sol, subagent/scout → Luna. Terra
// remains available only through an explicit pin. --frontier auto-detects the
// same Sol/Luna profile from model/list.
const DEFAULT_MODEL_ORCHESTRATOR = "gpt-5.6-sol";
const DEFAULT_MODEL_SUBAGENT = "gpt-5.6-luna";
const DEFAULT_MODEL_SCOUT = "gpt-5.6-luna";

// Resolve the per-role pinned models from CLI flags. --frontier is applied
// LATER (it requires a Codex call), so --plan can use these defaults without
// connecting.
let pinnedOrchestrator = opts.pinModelOrchestrator ?? (opts.pinModel ?? DEFAULT_MODEL_ORCHESTRATOR);
let pinnedSubagent = opts.pinModelSubagent ?? (opts.pinModel ?? DEFAULT_MODEL_SUBAGENT);
let pinnedScout = opts.pinModelScout ?? (opts.pinModel ?? DEFAULT_MODEL_SCOUT);

// `pinnedModel` is the legacy single-model pin; the runtime still uses it for
// back-compat (e.g. scripts that pass `model` opts we don't intercept).
...
if (opts.plan) {
  ...
  try {
    await runWorkflowFile(resolve(opts.script), {
      ...
      pinnedModelOrchestrator: pinnedOrchestrator,
      pinnedModelSubagent: pinnedSubagent,
      pinnedModelScout: pinnedScout,
      autoEffort: opts.autoEffort,
      pinnedEffort,
      pinnedEffortOrchestrator: opts.pinEffortOrchestrator ?? DEFAULT_EFFORT_ORCHESTRATOR,
      pinnedEffortSubagent: opts.pinEffortSubagent ?? DEFAULT_EFFORT_SUBAGENT,
      pinnedEffortScout: opts.pinEffortScout ?? DEFAULT_EFFORT_SCOUT,
      plan: true,
...
// --frontier: connect to Codex, auto-detect the Sol/Luna role profile, and
// override per-role pins. Only reached for live runs (--plan exits above).
if (opts.frontier) {
  try {
    const client = await getClient();
    const roles = pickFrontierRoles(await client.listModels());
    if (!roles.orchestrator || !roles.subagent || !roles.scout) {
      console.error("--frontier: could not determine a role profile from model/list");
      await shutdownClient();
      process.exit(1);
    }
    pinnedOrchestrator = roles.orchestrator;
    pinnedSubagent = roles.subagent;
    pinnedScout = roles.scout;
    if (opts.pinModelOrchestrator || opts.pinModelSubagent || opts.pinModelScout || opts.pinModel) {
      console.error("--frontier overrides --pin-model and all per-role model pins");
    }
    pinnedModel = undefined; // legacy path disabled by --frontier
  } catch (e) {
...
if (opts.pinModel) {
  console.error(`⊙ pinning all agents to model: ${opts.pinModel}`);
} else {
  console.error(
    `⊙ orchestrator model: ${pinnedOrchestrator}  ·  ` +
    `subagent model: ${pinnedSubagent}  ·  scout model: ${pinnedScout}`,
  );
}
...
if (!opts.noJournal) {
  ...
  try {
    writeFileSync(runMetaPathFor(journalPath), JSON.stringify({
      ...
      modelOrchestrator: pinnedOrchestrator,
      modelSubagent: pinnedSubagent,
      modelScout: pinnedScout,
      autoEffort: opts.autoEffort,
      pinEffort: pinnedEffort,
      pinEffortOrchestrator: opts.pinEffortOrchestrator ?? DEFAULT_EFFORT_ORCHESTRATOR,
      pinEffortSubagent: opts.pinEffortSubagent ?? DEFAULT_EFFORT_SUBAGENT,
      pinEffortScout: opts.pinEffortScout ?? DEFAULT_EFFORT_SCOUT,
