...
export async function codexAgent(prompt, opts = {}) {
  ...
  // Resolve the agent's role (default `subagent`). The per-role pinned model wins
  // when set; the legacy single `pinnedModel` (--pin-model M) wins for all roles
  // when set without per-role overrides — preserves the "one model everywhere"
// back-compat path. A per-call `model` and an agentType model are weaker.
  const role = opts.role === "orchestrator" || opts.role === "scout"
    ? opts.role
    : "subagent";
  const pins = {
    orchestrator: opts.pinnedModelOrchestrator,
    subagent: opts.pinnedModelSubagent,
    scout: opts.pinnedModelScout,
  };
  const rolePinned = pins[role] ?? opts.pinnedModel;
  const effectivePinned = rolePinned ?? opts.pinnedModel;
  if (effectivePinned && opts.model && opts.model !== effectivePinned) {
    log(`pinned model '${effectivePinned}' (role:${role}) overrides per-call model '${opts.model}'`);
  }
  const requestedModel = effectivePinned ?? opts.model ?? agentTypeModel ?? opts.defaultModel;
