...
export function createRuntime({
  ...
  defaults = {},
  defaultModel,
  pinnedModel, // legacy single-model pin (overrides all roles when set without per-role pins)
  pinnedModelOrchestrator = "gpt-5.6-sol", // orchestrator role (role:'orchestrator')
  pinnedModelSubagent = "gpt-5.6-luna", // subagent role (role:'subagent', the default)
  pinnedModelScout = "gpt-5.6-luna", // scout role (role:'scout')
  autoEffort = false,
  pinnedEffort = null, // legacy single-effort pin (overrides all roles when set without per-role pins)
  pinnedEffortOrchestrator = "ultra", // orchestrator role default
  pinnedEffortSubagent = "high", // subagent role default
  pinnedEffortScout = "medium", // scout role default
  ...
  // Resolve the role for one agent call. Default `subagent` (the common case);
  // scripts opt in to critical `orchestrator` or lightweight `scout` calls.
  function resolveRole(callOpts) {
    if (callOpts.role === "orchestrator" || callOpts.role === "scout") {
      return callOpts.role;
    }
    return "subagent";
  }
  // Per-role pinned model. The legacy single `pinnedModel` (--pin-model M) wins
  // for all roles when set without per-role overrides — it is the "force every
  // agent onto one model" escape hatch and a faithful back-compat path.
  function resolvePinnedModel(callOpts) {
    const role = resolveRole(callOpts);
    if (role === "orchestrator") return pinnedModelOrchestrator ?? pinnedModel;
    if (role === "scout") return pinnedModelScout ?? pinnedModel;
    return pinnedModelSubagent ?? pinnedModel;
  }
  // Thinking-effort policy in ONE place (used by agent() and sessions). The
  // per-role pins are the default baseline (orchestrator → ultra, subagent →
  // high, scout → medium); the legacy single `pinnedEffort` still wins for all
  // roles when set without per-role
  // overrides. `--auto-effort` is an explicit opt-in to layer-width scaling
  // that BEATS the per-role defaults (a script that passes `--auto-effort`
  // is saying "I want width-based scaling, not your role defaults"). Precedence
  // (highest first):
  //   pinnedEffort (--pin-effort)               legacy authoritative, all roles
  //   per-call opts.effort                      author's deliberate choice
  //   --auto-effort (layer width)               1→xhigh, 2+→high (floor); beats role pins
  //   pinnedEffort{Orchestrator,Subagent,Scout} per-role default
  //   defaults.effort (--effort)                flat fallback
  //   undefined                                 Codex config default
  function resolveEffort(callOpts, width) {
    ...
    const role = resolveRole(callOpts);
    let perRole = pinnedEffortSubagent;
    if (role === "orchestrator") perRole = pinnedEffortOrchestrator;
    if (role === "scout") perRole = pinnedEffortScout;
    if (perRole != null) return { effort: perRole, src: "role-pin" };
  ...
  async function agent(prompt, opts = {}) {
    ...
    //   pinnedEffort{Orchestrator,Subagent,Scout} per-role authoritative
    //   per-call opts.effort                      author's deliberate choice
    ...
    const rolePinnedModel = resolvePinnedModel(opts);
    ...
    if (plan) {
      onAgentPlan?.({ label, phase: effectivePhase, effort: merged.effort ?? null, width, schema: !!opts.schema, role, model: rolePinnedModel ?? null });
    ...
    // Identity includes the *effective* model (role-pinned, else script opt,
    // else CLI default) and effort (set above on `merged`) so a model/effort
    // change busts the cache; --fresh forces a full re-run.
    const effectiveModelForKey = rolePinnedModel ?? opts.model ?? defaultModel;
    const key = journal
      ? journal.nextKey(prompt, { ...merged, model: effectiveModelForKey })
      : null;
    ...
    const reqModel = rolePinnedModel ?? opts.model ?? defaultModel ?? null;
    const effortTag = merged.effort
      ? `  ⟪${merged.effort}${effortSrc === "auto" ? `·layer×${width}` : effortSrc === "role-pin" ? `·${role}` : ""}⟫`
      : "";
    onLog?.(`  · agent: ${label}${opts.role ? `  ⟨role:${opts.role}⟩` : ""}${opts.schema ? "  [schema]" : ""}${effortTag}`);
    ...
    onEvent?.({ type: "start", id: key, label, phase: effectivePhase, effort: merged.effort ?? null, model: reqModel, role });
    ...
    const result = await pooled(() =>
      runAgent(prompt, {
        ...merged, defaultModel, pinnedModel, pinnedModelOrchestrator, pinnedModelSubagent,
        pinnedModelScout, log: onLog,
  ...
  async function workflow(ref, subArgs) {
    ...
    return runWorkflowFile(scriptPath, {
      ...
      defaultModel,
      pinnedModel,
      pinnedModelOrchestrator,
      pinnedModelSubagent,
      pinnedModelScout,
      autoEffort,
      pinnedEffort,
      pinnedEffortOrchestrator,
      pinnedEffortSubagent,
      pinnedEffortScout,
      plan,
      onPhase,
      onLog,
  ...
  class LiveAgentSession {
    constructor({ id, driver, label, phase, reqModel, effort, replay = null, role = "subagent" }) {
      this.id = id;
      this.label = label;
      this.phase = phase;
      this.role = role;
      ...
      this._snapshot = {
        ...
        model: reqModel, effort: this._effort, tokens: null, ms: null, role,
      };
    }
    ...
    async _beginTurn(prompt, turnOpts, kind) {
      ...
      if (cached && promptMatches) {
        ...
        this._snapshot = {
          ...
          tokens: cached.tokens ?? null, ms: cached.ms ?? null, role: this.role,
        };
        onLog?.(`  ◦ ${kind === "steer" ? "steer" : "agent.start"} (cached): ${this.label}`);
        onEvent?.({
          type: "cached", id: sessKey, label: this.label, phase: this.phase,
          role: this.role, kind: "session", sessionId: this.id, turn: turnIndex,
        });
      ...
      onEvent?.({
        ...
        effort: effort ?? null, model: this._reqModel, role: this.role,
    ...
    async _settleTurn(outcome, { sessKey, turnIndex, effort, promptHash }) {
      ...
      try {
        onEvent?.({
          ...
          effort: effort ?? null, model: this._snapshot.model, role: this.role,
      ...
      if (journal) {
        try {
          await journal.record(sessKey, this.label, this._snapshot.result ?? this._snapshot.text ?? null, {
            ...
            tokens: this._snapshot.tokens, ms: this._snapshot.ms, role: this.role,
  ...
  async function startLiveSession(prompt, opts = {}) {
    ...
    try {
      driver = await startSession({
        ...merged, defaultModel, pinnedModel, pinnedModelOrchestrator,
        pinnedModelSubagent, pinnedModelScout, log: onLog,
    ...
    const session = new LiveAgentSession({ id, driver, label, phase, reqModel, effort, replay, role });
  ...
  class PlannedAgentSession {
    constructor({ id, label, phase, reqModel, effort, role = "subagent", schema }) {
      this.id = id;
      this.label = label;
      this.phase = phase;
      this.role = role;
    ...
    _snap(schema, status, turnId) {
      return {
        ...
        model: this._reqModel, effort: this._effort, role: this.role, tokens: null, ms: null,
      };
    }
    get status() { return this._status; }
    ...
    async steer(message, opts = {}) {
      ...
      const turnIndex = this._turnCount++;
      onAgentPlan?.({
        label: this.label, phase: this.phase,
        effort: (opts.effort ?? this._effort) ?? null,
        width: 1, schema: !!opts.schema, role: this.role,
        model: this._reqModel, kind: "steer",
      });
  ...
  async function startPlannedSession(prompt, opts = {}) {
    ...
    const id = `s${++sessionSeq}`;
    bumpAgentCount();
    onAgentPlan?.({ label, phase, effort: effort ?? null, width, schema: !!opts.schema, role, model: reqModel, kind: "session-start" });
    return new PlannedAgentSession({ id, label, phase, reqModel, effort, role, schema: opts.schema });
  }
