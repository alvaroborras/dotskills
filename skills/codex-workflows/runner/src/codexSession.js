...
export async function startCodexSession(opts = {}) {
  ...
  // Resolve the session's role (default `subagent`). The per-role pinned model
// wins when set; the legacy single `pinnedModel` (--pin-model M) is the
// back-compat fallback. Same precedence as agent().
