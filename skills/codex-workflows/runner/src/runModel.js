...
export function liveState(events) {
  ...
  for (const e of events) {
    ...
    if (e.type === "start") {
      c.starts++; c.lastStartT = e.t ?? c.lastStartT; c.phase = e.phase; c.model = e.model; c.effort = e.effort; c.role = e.role;
  ...
  for (const [id, c] of byId) {
    if (c.starts > c.ends) {
      const r = { id, label: c.label, phase: c.phase ?? null, model: c.model ?? null, effort: c.effort ?? null, role: c.role ?? null, startedAt: c.lastStartT, status: "running" };
...
function attachSessions(run) {
  ...
  for (const a of run.agents) {
    ...
    const s = byId.get(a.sessionId) || {
      id: a.sessionId, label: a.label, phase: a.phase, model: null, effort: null, role: null,
...
export function buildLiveRunModel(opts) {
  ...
  if (ls && ls.running.length) {
    ...
    for (const r of ls.running) {
      ...
      const a = { id, label: r.label, order: order++, phase, model: r.model ?? null, effort: r.effort ?? null, role: r.role ?? null, tokens: null, ms: null, result: undefined, status: "running", startedAt: r.startedAt };
