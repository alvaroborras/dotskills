  ...
  const summary = {
    ...
    policy: meta
      ? {
          ...
          modelOrchestrator: meta.modelOrchestrator ?? null,
          modelSubagent: meta.modelSubagent ?? null,
          modelScout: meta.modelScout ?? null,
          autoEffort: !!meta.autoEffort,
          pinEffort: meta.pinEffort ?? null,
          pinEffortOrchestrator: meta.pinEffortOrchestrator ?? null,
          pinEffortSubagent: meta.pinEffortSubagent ?? null,
          pinEffortScout: meta.pinEffortScout ?? null,
...
export function renderSummaryText(s, { includeResult = false } = {}) {
  ...
  // policy
  if (s.policy) {
    const p = s.policy, lines = [];
    // Per-role model/effort. Older journals may not have scout metadata, so
    // render it only when present and preserve the two-role summary shape.
    if (p.modelOrchestrator && p.modelSubagent) {
      const orchEff = p.pinEffortOrchestrator ? "effort " + p.pinEffortOrchestrator : null;
      const subEff = p.pinEffortSubagent ? "effort " + p.pinEffortSubagent : null;
      lines.push("orchestrator: " + p.modelOrchestrator + (orchEff ? "  (" + orchEff + ")" : ""));
      lines.push("subagent:     " + p.modelSubagent + (subEff ? "  (" + subEff + ")" : ""));
      if (p.modelScout) {
        const scoutEff = p.pinEffortScout ? "effort " + p.pinEffortScout : null;
        lines.push("scout:        " + p.modelScout + (scoutEff ? "  (" + scoutEff + ")" : ""));
      }
    } else if (p.model) {
      lines.push("model: " + p.model);
    }
    if (p.autoEffort) lines.push("auto-effort");
    if (p.pinEffort) lines.push("pin-effort " + p.pinEffort);
    if (p.sandbox) lines.push("sandbox " + p.sandbox);
    if (lines.length) {
      L.push(""); L.push(section("Run policy"));
...
export function renderSummaryMarkdown(s, { includeResult = false } = {}) {
  ...
  if (s.policy) {
    const p = s.policy, lines = [];
    if (p.modelOrchestrator && p.modelSubagent) {
      const orchEff = p.pinEffortOrchestrator ? " · effort `" + p.pinEffortOrchestrator + "`" : "";
      const subEff = p.pinEffortSubagent ? " · effort `" + p.pinEffortSubagent + "`" : "";
      lines.push("- orchestrator: `" + p.modelOrchestrator + "`" + orchEff);
      lines.push("- subagent:     `" + p.modelSubagent + "`" + subEff);
      if (p.modelScout) {
        const scoutEff = p.pinEffortScout ? " · effort `" + p.pinEffortScout + "`" : "";
        lines.push("- scout:        `" + p.modelScout + "`" + scoutEff);
      }
    } else if (p.model) {
      lines.push("- model `" + p.model + "`");
    }
    ...
    if (p.pinEffort) lines.push("- pin-effort `" + p.pinEffort + "`");
    if (p.sandbox) lines.push("- sandbox `" + p.sandbox + "`");
    if (lines.length) L.push("", "## Run policy", "", ...lines);
  }
  if (includeResult && s.result !== undefined) {
