...
// Build the candidate table once: parse version + exclude -mini/-spark/hidden.
function _frontierCandidates(models = []) {
...
export function pickFrontier(models = []) {
  const eligible = _frontierCandidates(models);
  return eligible[0]?.id;
}

// Pick the default role profile. Prefer the latest exposed `-sol` model for
// critical orchestrator gates and the latest exposed `-luna` model for both
// regular workers and cheap scouts. Terra is never selected automatically; it
// remains available through explicit per-role pin flags. If Luna is absent,
// use the next non-Terra frontier model, then fall back to the orchestrator.
export function pickFrontierRoles(models = []) {
  const eligible = _frontierCandidates(models);
  if (!eligible.length) {
    return { orchestrator: undefined, subagent: undefined, scout: undefined };
  }

  const latestVersion = modelVersion(eligible[0].id);
  const latest = eligible.filter((model) => modelVersion(model.id) === latestVersion);
  const orchestrator = latest.find((model) => /-sol$/i.test(model.id))?.id
    ?? eligible.find((model) => !/-terra$/i.test(model.id))?.id;
  if (!orchestrator) {
    return { orchestrator: undefined, subagent: undefined, scout: undefined };
  }
  const luna = latest.find((model) => /-luna$/i.test(model.id))?.id;
  const fallbackWorker = eligible.find(
    (model) => model.id !== orchestrator && !/-terra$/i.test(model.id),
  )?.id;
  const worker = luna ?? fallbackWorker ?? orchestrator;

  return { orchestrator, subagent: worker, scout: worker };
}

// Back-compatible paired view for callers that have not adopted the scout role.
export function pickFrontierPair(models = []) {
  const { orchestrator, subagent } = pickFrontierRoles(models);
