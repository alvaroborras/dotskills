...
import { resolveModel, pickFrontier, pickFrontierRoles } from "../src/modelMap.js";
...
// 11) frontier selection: newest non-mini/spark general model.
{
  ...
  assert.deepEqual(
    pickFrontierRoles(["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"]),
    {
      orchestrator: "gpt-5.6-sol",
      subagent: "gpt-5.6-luna",
      scout: "gpt-5.6-luna",
    },
    "role profile routes Sol to orchestrators and Luna to workers/scouts",
  );
  assert.deepEqual(
    pickFrontierRoles(["gpt-5.6-sol", "gpt-5.6-terra"]),
    {
      orchestrator: "gpt-5.6-sol",
      subagent: "gpt-5.6-sol",
      scout: "gpt-5.6-sol",
    },
    "Terra is never selected automatically when Luna is unavailable",
  );
  assert.deepEqual(
    pickFrontierRoles(["gpt-5.6-terra"]),
    { orchestrator: undefined, subagent: undefined, scout: undefined },
    "a Terra-only model list cannot satisfy the automatic role policy",
  );
}

// 11b) default model routing preserves distinct subagent/scout attribution even
//      though both roles use Luna.
{
  const planned = [];
  await runWorkflowSource(
    [
      'export const meta = { name: "roles" };',
      'await agent("orchestrate", { role: "orchestrator" });',
      'await agent("work");',
      'await agent("scan", { role: "scout" });',
      'return true;',
    ].join("\n"),
    { plan: true, onAgentPlan: (record) => planned.push(record) },
  );
  assert.deepEqual(
    planned.map(({ role, model }) => ({ role, model })),
    [
      { role: "orchestrator", model: "gpt-5.6-sol" },
      { role: "subagent", model: "gpt-5.6-luna" },
      { role: "scout", model: "gpt-5.6-luna" },
    ],
  );
}
...
// 13) effort precedence: pin > per-call > auto > --effort flag > omitted.
{
  ...
  const r3 = await runWorkflowSource(
    ...
    { defaults: { effort: "medium" }, pinnedEffortSubagent: null, runAgent: echo },
  );
  assert.equal(r3, "medium", "with per-role pins disabled, --effort is the fallback");

  const r4 = await runWorkflowSource(
    'export const meta = { name: "p4" }; return await agent("x");',
    { pinnedEffortSubagent: null, runAgent: echo },
  );
  assert.equal(r4, "(none)", "no effort anywhere -> omitted (Codex config default)");

  // Per-role defaults: orchestrator -> ultra, subagent -> high, scout -> medium.
  const r5 = await runWorkflowSource(
    'export const meta = { name: "p5" }; return await agent("x", { role: "orchestrator" });',
    { runAgent: echo },
  );
  assert.equal(r5, "ultra", "orchestrator role defaults to ultra");
  const r6 = await runWorkflowSource(
    'export const meta = { name: "p6" }; return await agent("x");',
    { runAgent: echo },
  );
  assert.equal(r6, "high", "subagent role (default) -> high");
  const rScout = await runWorkflowSource(
    'export const meta = { name: "p-scout" }; return await agent("x", { role: "scout" });',
    { runAgent: echo },
  );
  assert.equal(rScout, "medium", "scout role defaults to medium");
  // --auto-effort still beats per-role defaults.
  const r7 = await runWorkflowSource(
    'export const meta = { name: "p7" }; return await agent("x");',
    { autoEffort: true, runAgent: echo },
  );
  assert.equal(r7, "xhigh", "--auto-effort beats per-role defaults; lone agent -> xhigh");
}
