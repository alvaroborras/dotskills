...
Read `../../autoresearch/references/shared-contract.md` first. Its fail-closed
verification, candidate isolation, Git safety, campaign, and lifecycle rules
override conflicting examples below.
...
Autonomous fix loop that takes a broken state and iteratively repairs it until everything passes. One isolated, verified fix per iteration.

**Core idea:** Detect  Prioritize  Fix ONE thing  Verify  Promote/Abandon  Repeat until zero errors.
...
```
# Unbounded budget  continue while lifecycle is RUNNING and failures remain
/autoresearch:fix
...
**Blocking prerequisite:** If target, guard, or scope remains unresolved after failure discovery, use the current environment's available clarification mechanism before candidate changes.
...
When clarification is required, batch these four questions in one interaction when supported:
...
**Gate:** Do not enter this phase until target, guard, and scope are resolved.
...
## Phase 4: Freeze the Candidate Before Verification

Record the exact candidate diff and source hash in its isolated workspace. Do not
stage unrelated paths or commit an unverified candidate. The isolated workspace is
...
## Phase 7: Decide  Keep, Discard, or Rework

| Condition | Action |
|-----------|--------|
| `delta > 0` AND guard passes | **KEEP**  parent may promote/commit the verified candidate, log "fixed" |
| `delta > 0` AND guard fails | **REWORK**  preserve evidence and abandon the isolated candidate (max 2 attempts) |
| `delta == 0` | **DISCARD**  preserve evidence and abandon the isolated candidate |
| `delta < 0` (more errors!) | **DISCARD**  preserve evidence and abandon the isolated candidate |
| Crash during fix | **RECOVER**  preserve evidence, abandon candidate, try simpler approach (max 3 attempts) |

**Rework strategy (when guard fails):**
1. Read the guard failure and understand what regressed.
2. Save the candidate diff and verification evidence, then abandon its isolated
   workspace without touching unrelated user work.
3. Understand why the fix broke something else (check cascading dependencies).
4. Create a fresh isolated candidate that fixes the target without the regression.
5. If 2 rework attempts fail, record a blocked event and move to the next item.
6. Render the `rework` result in the generated results view.
...
**Append a ledger event and render this `fix-results.tsv` view:**
```tsv
...
FIXING:
   ...
   Freeze candidate diff/hash  VERIFYING
...
DECIDING:
   ...
   delta == 0  DISCARD isolated candidate  PRIORITIZING (next item)
   delta < 0  DISCARD isolated candidate  PRIORITIZING
...
FUNCTION detectCompound(before_errors, after_errors):
  ...
  IF delta == 0 AND error_details_changed:
    ...
    PRESERVE evidence, abandon isolated candidate, and try a new approach
```
...
## Safe Abandonment Protocol

When a fix makes things worse or breaks the guard:

1. Stop evaluation and record the typed result and affected guard.
2. Abandon the disposable worktree/snapshot, or perform only a targeted safe revert
   when the experiment mode and repository state permit it.
3. Verify that the protected baseline worktree and its dirty-path hashes are
   unchanged.
4. Record `discard`; if safe recovery cannot complete, record `rollback-blocked`,
   preserve the diff, and stop mutation.
5. Analyze the wrong assumption before creating a materially new candidate.

Never use broad checkout, reset, clean, automatic stashing, or lock deletion to
recover. An isolated failed candidate must not contaminate the next baseline.
...
- api.test.ts  fix expected status code 200201 (test)
[... 41 more ...]
