...
Before each iteration, build situational awareness. **You MUST complete ALL 8 steps  git history and durable state are critical for learning from past iterations.**

```
1. Read `autoresearch-state.md` for the compact current state
2. Read `autoresearch-ideas.md` for untried and tried approaches
3. Read current state of in-scope files (only the sections needed for the next decision)
4. Read last 10-20 entries from `autoresearch-results.tsv` and `autoresearch-decisions.tsv`
5. MUST run: git log --oneline -20 to see recent changes
6. MUST run: git diff HEAD~1 (if last iteration was "keep") to review what worked
7. Identify: what worked, what failed, what's untried  based on state files, results log, AND git history
8. If bounded: check current_iteration vs max_iterations
```

**Why read git history and state files every time?** Git IS the audit trail. The compact files are the active working memory. After rollbacks, state may differ from what you expect. The git log shows which experiments were kept vs reverted; `autoresearch-ideas.md` prevents retrying discarded ideas under new names; `autoresearch-state.md` keeps the current metric and next hypothesis out of conversation history. Never assume  always verify.
...
Use subagents to reduce uncertainty without bloating the main context. They are helpers, not decision makers.

**Dispatch subagents for:**
- Codebase scouting across disjoint files or subsystems.
- Independent syntax checks, diff review, or output analysis while a harness run is in flight.
- Disjoint file edits when each agent owns a separate explicit file and the main agent will integrate.

**Do not dispatch subagents for:**
- The canonical harness run. Use pi-tasks or native async bash for that.
- The keep/discard decision. The main agent compares the metric, guard result, and complexity tradeoff.
- Final reporting.

Every subagent assignment must include a hard output cap such as `return ≤15 lines, no logs, only actionable deltas`. Prefer artifact paths for large output.

Generic scouting pattern:

```text
task(agent="explore", tasks=[
  {id: "ScoutCore", assignment: "Read src/core/algo.py and return ≤15 lines: likely optimization targets only."},
  {id: "ScoutPipeline", assignment: "Read src/pipeline/stage2.py and return ≤15 lines: hot paths or dead branches only."}
])
```
...
|-----------------|---------------------|
| Change port 30008080 in Dockerfile + compose + nginx | Change port AND add new service |
| Update Node 1820 in Dockerfile + CI + package.json | Update Node AND switch to pnpm |
...
FILES_TO_STAGE=(src/api/users.ts)
...
## Phase 4.5: Async Execute Long Harnesses

Use async execution for long verification or benchmark commands so the main context can keep doing independent work instead of carrying repeated polling chatter.

**Tier 1 — Preferred: `@tintinweb/pi-tasks`**

When pi-tasks is installed, create a structured task for the harness command, execute it in the background, and retrieve output once when needed:
...
# While it runs: read state files, prepare the next hypothesis candidate, or dispatch output-analysis scouts.
TaskOutput(task_id="#1", block=true, timeout=<timeout-ms>)
TaskUpdate(task_id="#1", metadata={hypothesis: "<one sentence>", result: "<metric>", decision: "<keep|discard|crash>"})
```

Use project-scoped pi-tasks persistence so experiment metadata survives restarts. Model dependencies with pi-tasks only when an experiment truly depends on another experiment's result.

**Tier 2 — Fallback: native async bash**

If pi-tasks is unavailable, use native async execution and wait once at the end:

```text
bash(async: true, timeout: <seconds>, command: "<verify-or-harness-command>")
# While it runs: perform independent analysis and update state files.
job(poll: ["<job-id>"])  # final blocking wait only, never a polling loop
```

Do not paste full logs into the conversation. Keep the job/artifact reference, extract the metric, and store durable conclusions in `autoresearch-state.md`, `autoresearch-results.tsv`, and `autoresearch-decisions.tsv`.
...
42         a1b2c3d  0.9821   keep          increase cache size for hot requests
43         -        0.9845   discard       switch optimizer to alternate strategy
...
Append the matching compact decision row to `autoresearch-decisions.tsv`:

```tsv
iter	hypothesis	result	decision	notes
42	increase cache size for hot requests	0.9821	keep	improved median latency
43	switch optimizer to alternate strategy	0.9845	discard	regressed metric
```

Update `autoresearch-state.md` with current best, iteration count, active hypothesis, and last result. Move tried hypotheses in `autoresearch-ideas.md` from `[untried]` to `[tried-good]` or `[tried-bad]`; add newly discovered ideas without explaining them in conversation.
