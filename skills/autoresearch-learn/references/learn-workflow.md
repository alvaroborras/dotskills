...
Read `../../autoresearch/references/shared-contract.md` first. Its capability,
delegation, state, lifecycle, and context rules override conflicting examples below.
...
**Blocking prerequisite:** If mode or essential context cannot be inferred safely, use the current environment's available clarification mechanism before Phase 1.

**Capability rule:** Discover the clarification capabilities actually exposed by the host. If none exists, ask through ordinary conversation or report the blocker; never invent a tool.
...
### Questions (one compact clarification batch when supported)
...
/autoresearch:learn
   ...
   Phase 4: Generate  Use parent or bounded available writer
...
2. **Exclusion list:** discovered host-agent config directories, `.git`, `node_modules`, `__pycache__`, `secrets`, `vendor`, `dist`, `build`, `.next`, `.nuxt`, `coverage`, `generated`, `*.min.js`, `.cache`, `.tmp`
...
4. Discover available read-only scouting capabilities; use bounded parallel scouts
   only when the host exposes them, otherwise scout locally.
...
The parent/controller owns delegation. First discover whether bounded read-only
scouts are available; otherwise read locally.
...
4. **Strategy by count:**
   - **0 files:** Ask through the available clarification mechanism whether to switch to init mode; cancel cleanly otherwise.
   - **1-3 files:** Read directly.
   - **4-6 files:** Use 2-3 available read-only scouts when useful.
   - **7+ files:** Use up to 5 available read-only scouts when useful.
5. **Context budget:** If total docs LOC >5000, require the writer assignment
   to summarize before generation
6. Distribute by LOC (larger files get dedicated agent)
7. **Read-only scout output contract:**
   ```
   ...
   **Needs update:** areas likely stale or incomplete based on content
   ...
   Files: {list}"
   ```
8. Parent verifies cited files and merges scout results into context.
...
- Instruct the selected writer (parent or bounded isolated child) to focus
  regeneration on mapped docs and use a light touch elsewhere
...
## Phase 4: Generate with an Available Writer

The parent may write directly or delegate a bounded writer only after capability
discovery. A delegated writer receives explicit file ownership, uses an isolated
workspace, and cannot decide canonical validity or promotion. Never assume a named
agent or task API.
...
2. **Doc reading results** (from Phase 3 read-only scouts, update mode only)
...
1. Discover a repository-provided documentation validator or host capability.
2. If available, run it with an exact recorded command against the generated docs.
3. Check that code references exist, internal links resolve, config keys are real,
   and Markdown syntax is valid.
4. Display the validation report with warnings.
5. If unavailable, run the manual checks and record the limitation; never invent a
   host-specific validator path.
...
LOOP:
  ...
  2. Re-run the parent writer or available bounded writer with:
...
Append a ledger event, then render this `learn-results.tsv` report view:
