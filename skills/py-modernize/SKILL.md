...
Use this skill for focused modernization work. For broad refactoring that spans modernization plus testing, security, or complexity work, prefer `py-refactor`.

In Codex, prefer `uv sync` and `uv run <tool>` over manually activating virtual environments in shell snippets.
...
**Install uv via your package manager if needed.** In Codex, prefer `uv sync`, `uv run`, `uv pip`, and `uv venv`.
