...
delegation, state, lifecycle, and evidence rules override conflicting examples below.
...
Multi-persona prediction that pre-analyzes code from several expert perspectives.
Use available host capabilities to run or simulate 3–5 independent analyses, debate,
and consensus, producing ranked findings and hypotheses without assuming a vendor.
...
```
# Unbounded budget  continue only while lifecycle remains RUNNING
...
**Blocking prerequisite:** If scope, goal, or depth cannot be inferred safely, use the current environment's available clarification mechanism before analysis.

**Capability rule:** Discover the clarification capabilities exposed by the host. If none exists, ask through ordinary conversation or report the blocker; never invent or fetch an unavailable tool.
...
When clarification is required, ask the selected questions in one compact batch when supported:
...
**Important:** Prefer one compact batch so the user can make related decisions together.
...
**Gate:** Do not enter this phase until scope, goal, and depth are resolved.
...
The parent/controller reads all in-scope source files and writes structured knowledge files that personas will reference. This prevents redundant rereading and gives each persona a consistent shared context.
...
2. Flag in overview.md: ` Anti-herd detection: high convergence detected. Minority findings may be underweighted.`
...
If hashes differ:
```
 Staleness Warning: Knowledge files were built from {cached_hash} but HEAD is now {current_hash}.
...
Reports older than 30 days also receive: ` Age Warning: This report is {N} days old.`
